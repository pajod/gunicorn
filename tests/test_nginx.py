...
def test_nginx_proxy(
    *,
    ssl: bool,
    worker_class: str,
    dummy_ssl_cert: tuple[Path, Path],
    read_size: int = 1024,
):
    ...
    static_dir = Path("/run/gunicorn/nonexist")
    ...
    with (
        TemporaryDirectory(suffix="_temp_py") as tempdir_name,
        StdlibClient(nginx_bind) as client,
    ):
