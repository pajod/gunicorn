...
def test_arbiter_reexec_not_using_path(execve, fork, getpid):
    arbiter = gunicorn.arbiter.Arbiter(DummyApplication())
    listener1 = mock.Mock()
    listener1.fileno.return_value = 4
    arbiter.LISTENERS = [listener1]
    fork.return_value = 0
    getpid.side_effect = [2, 3]
    arbiter.reexec()
    exec_path = execve.call_args[0][0]
    arg0_path = execve.call_args[0][1][0]
    assert exec_path == arg0_path
    assert os.path.isabs(arg0_path)
