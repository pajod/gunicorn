...
CMD_OPENSSL: str | None = shutil.which("openssl")
CMD_WRK: str | None = shutil.which("wrk")
...
PY_ASGI = f"""
import time
async def {APP_FUNC_NAME}(scope, receive, send):
...
DEP_WANTED: set[str] = set(chain(*WORKER_DEPENDS.values()))
DEP_INSTALLED: set[str] = set()
...
class SubProcess(subprocess.Popen[bytes]):
    GRACEFUL_SIGNAL: signal.Signals = signal.SIGQUIT
    EXIT_SIGNAL: signal.Signals = signal.SIGINT
    _argv: list[str]
    temp_path: Path
    ...
    def graceful_quit(
        self, expect: set[str] | None = None, ignore: list[str] | None = None
    ) -> str:
        assert self.stdout is not None
        assert self.stderr is not None
...
class NginxProcess(SubProcess):
    ...
    CMD_NGINX: str | None = shutil.which("nginx", path=_PATH)

    @classmethod
    def gen_config(
        cls, *, bind: str, temp_path: Path, upstream: str, static_dir: Path, ssl: bool
    ) -> str:
    ...
    def __init__(
        ...
        *,
        ...
        assert self.CMD_NGINX is not None
...
def generate_dummy_ssl_cert(cert_path: Path, key_path: Path):
    # dummy self-signed cert
    assert CMD_OPENSSL is not None
...
def dummy_ssl_cert(tmp_path_factory) -> tuple[Path, Path]:
...
class GunicornProcess(SubProcess):
    ...
    def __init__(
        ...
        *,
        ...
        server_bind: str,
        read_size: int = 1024,
        ssl_files: tuple[Path, Path] | None = None,
...
class StdlibClient:
    ...
    def get(self, path: str = "/", test: bool = False) -> http.client.HTTPResponse:
        body = b"GETBODY!"
        self.conn.request(
            "GET",
            path,
...
class WrkClient(subprocess.Popen[bytes]):
    ...
    def __init__(self, url_base: str, path: str):
        assert path.startswith("/")
        assert CMD_WRK is not None
        threads = 10
        connections = 100
    ...
    def get(self) -> str:
        assert self.stdout is not None
        out: bytes = self.stdout.read(1024 * 4)
        ret = self.wait()
        assert ret == 0, ret
