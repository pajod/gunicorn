...
- fix :pr:`835`: display correct pid of already running instance
...
- fix :pr:`838`: statsd logger, send statsd timing metrics in milliseconds
- fix :pr:`839`: statsd logger, allows for empty log message while pushing
  metrics and restore worker number in DEBUG logs
- fix :pr:`850`: add timezone to logging
- fix :pr:`853`: Respect logger_class setting unless statsd is on
...
- :pr:`799`: fix support loading config from module
...
- add statsd logging handler fix :pr:`748`


Paster
++++++

- fix :pr:`809`: Set global logging configuration from a Paste config.


Extra
+++++

- fix RuntimeError in gunicorn.reloader (:pr:`807`)
...
version improve a lot the usage of Gunicorn with python 3 by adding :ref:`two
new workers <asyncio-workers>`
